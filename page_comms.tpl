			...
			<div class="card-body">
				...
				<table align="center" border="0" style="border-collapse:collapse;" id="group.details" cellpadding="3" class="table">
					<tr>
						<td valign="top">
							<div class="rowdesc">
								<i class="text-primary mdi mdi-help-circle" data-toggle="tooltip" data-placement="bottom" title="Wpisz tekst, który chcesz powiedzieć."></i> Comment
							</div>
						</td>
					</tr>
					<tr>
...
<div class="row">
	<div class="col-lg-12 grid-margin">
		<div class="card">
			<div class="card-body">
				<h4 class="lead">
				Podgląd na zablokowane komunikacje <a class="btn btn-outline-primary btn-rounded btn-fw" style="width:20px;height:20px;padding:0px;line-height:18px;" href="index.php?p=commslist&hideinactive={if $hidetext == 'Hide'}true{else}false{/if}{$searchlink|htmlspecialchars}" title="{$hidetext} Inactive">{$hidetext} Inactive</a></h3>
				<p>Liczba: {$total_bans}</p>
				...
				<div id="banlist" class="table-responsive">
					<div class="col-12 my-2 text-xl-right text-lg-left">
						<div id="banlist-nav" class="btn btn-inverse-light  btn-rounded btn-fw p-1 p-md-2 p-xl-2">
							...
							<select name="bulk_action" id="bulk_action" onchange="BulkEdit(this,'{$admin_postkey}');" class="btn btn-outline-primary btn-rounded btn-fw"
								...
								{if $general_unban}
								<option value="CU">Odblokuj</option>
								{/if}
								{if $can_delete}
								<option value="CD">Usuń</option>
								{/if}
							</select>
							{/if}
						</div>
					...
					<table class="table table-hover tbl-sm">
						<thead>
							<tr>
								...
								{/if}
								<th width="12%" class="text-center">MOD/Typ</th>
								<th width="14%" class="text-center">Data</th>
								<th>Gracz</th>
								{if $view_comments}
								<th class="text-right">Poprzednie blokady</th>
								{/if}
								{if !$hideadminname}
									...
									<table class="table tbl-sm" width="100%">
										<tr>
											...
											<td align="left" class="listtable_top" colspan="2">
												{/if}
												<b>Szczegóły Blokad</b>
											</td>
										</tr>
										<tr align="left">
											<td width="20%">Gracz</td>
											<td>
												{if empty($ban.player)}
												<i><font color="#677882">nie znalezioni nicku</font></i>
												{else}
												{$ban.player|escape:'html'|stripslashes}
												{/if}
											</td>
										...
										<tr align="left">
											...
											<td>
												...
												<i><font color="#677882">nie znalezioni steamid</font></i>
												{else}
												{$ban.steamid}
												{/if}
											</td>
										...
										<tr align="left">
											<td width="20%">Nadano</td>
											<td>{$ban.ban_date}</td>
										</tr>
										<tr align="left">
											<td width="20%">Długość blokady</td>
											<td>{$ban.banlength}</td>
										</tr>
										{if $ban.unbanned}
										<tr align="left">
											<td width="20%">Powód odblokowania</td>
											<td>
												{if $ban.ureason == ""}
										...
										<tr align="left">
											<td width="20%">Odblokowane przez</td>
											<td>
												{if !empty($ban.removedby)}
												{$ban.removedby|escape:'html'}
												{else}
												<i><font color="#677882">Admin usuniety</font></i>
												{/if}
											</td>
										</tr>
										{/if}
										...
										{if !$hideadminname}
										<tr align="left">
											<td width="20%">Zablokowany przez</td>
											<td>
												{if !empty($ban.admin)}
												{$ban.admin|escape:'html'}
												{else}
					...
					<div class="col-12 my-2 text-xl-right text-lg-left">
						<div id="banlist-nav" class="btn btn-inverse-light  btn-rounded btn-fw p-1 p-md-2 p-xl-2">
							{$ban_nav|replace:'|':''}  {if $view_bans} 
							<button type="button" class="btn btn-outline-primary btn-rounded btn-fw" style="height:24px;padding: 2px 10px; min-width:85px;" 
								onclick="TickSelectAll();return false;" title="Select All" name="tickswitchlink2" id="tickswitchlink2">Wybierz Wszystko</button>
							{/if}
							{if $general_unban || $can_delete}
							<select name="bulk_action" id="bulk_action" onchange="BulkEdit(this,'{$admin_postkey}');" class="btn btn-outline-primary btn-rounded btn-fw"
								style="min-width: auto; height: 24px; padding: 0px 12px;">
								<option value="-1">Działanie</option>
								{if $general_unban}
								<option value="CU">Odblokowanie</option>
								{/if}
								{if $can_delete}
								<option value="CD">Usuń</option>
								{/if}
